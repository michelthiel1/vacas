...
$stmt = $producao->read($searchQuery);

$message = $_SESSION['message'] ?? '';
unset($_SESSION['message']);
?>
<h2 class="page-title">Produção de Leite</h2>
    ...
    <form id="filter-form" method="GET" action="index.php">
        <div class="top-filter-bar">
            ...
            <div class="filter-buttons-group">
                 <a href="create.php" class="btn btn-primary">+ Novo Registro</a>
    ...
    <tbody>
        <?php if ($stmt->rowCount() > 0): ?>
            <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)): ?>
                <tr>
                    <td><?php echo date('d/m/Y', strtotime($row['data_producao'])); ?></td>
                    <td><?php echo htmlspecialchars($row['brinco_gado']); ?></td>
                    <td><?php echo htmlspecialchars(number_format($row['producao_total'], 2, ',', '.')); ?></td>
                    <td class="actions-column-condensed">
                        <div class="button-group">
                            <a href="edit.php?id=<?php echo $row['id']; ?>" class="btn btn-secondary">Editar</a>
                            <form action="../../controllers/ProducaoLeiteController.php" method="POST" onsubmit="return confirm('Tem certeza?');" style="display:inline-block;">
